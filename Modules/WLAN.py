...
class Wlan:
    ...
    def reported_tech_results(self):
        try:
            ...
            new_reported_tech_results = [drop_zero[blank].dropna(subset = ["Area Scan Max. SAR (W/kg)"]) for blank in range(len(drop_zero))]
    ...
    def spatial_sum_sar(self):
        try:
            ...
            new_reported_tech_results = [make_zeros[blank].dropna(subset = ["Area Scan Max. SAR (W/kg)"]) for blank in range(len(make_zeros))]
            ...
            try:
                abs_filepath_rrf = Path(f"{self.sssf}").resolve(strict = True)
